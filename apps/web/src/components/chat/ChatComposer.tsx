...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const savedPromptDraftRef = useRef("");
  ...
  const onComposerCommandKey = (
    ...
    if (key === "ArrowUp" && totalHistory > 0) {
      ...
      const isNavigatingHistory = promptHistoryIndexRef.current >= 0;
      if (!isNavigatingHistory && currentPrompt.trim().length > 0) {
        return false;
      }

      if (!isNavigatingHistory) {
        savedPromptDraftRef.current = currentPrompt;
    ...
    if (key === "ArrowDown" && promptHistoryIndexRef.current >= 0) {
      if (promptHistoryIndexRef.current < totalHistory - 1) {
        promptHistoryIndexRef.current += 1;
        const entry = historyAt(promptHistoryIndexRef.current);
        if (entry !== undefined) {
          promptRef.current = entry;
        ...
        promptHistoryIndexRef.current = -1;
        const draft = savedPromptDraftRef.current;
        promptRef.current = draft;
        setPrompt(draft);
        const nextCursor = collapseExpandedComposerCursor(draft, draft.length);
        setComposerCursor(nextCursor);
        setComposerTrigger(draft.length > 0 ? detectComposerTrigger(draft, draft.length) : null);
        window.requestAnimationFrame(() => {
          composerEditorRef.current?.focusAt(nextCursor);
        });
      }
